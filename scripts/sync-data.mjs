...
import { loadWorkbook } from './excel-workbook.mjs';
...
if (!candidates.length) {
  throw new Error('Nenhum arquivo XLSX encontrado em public/source/.');
}
...
const readSheet = (name) => workbook.readSheet(name);
...
const isoDate = (value) => {
  ...
  if (typeof value === 'number') {
    const parsed = workbook.parseDateCode(value);
...
const daily = readSheet('Operação Diária')
  .map((row) => {
...
if (!daily.length) {
  throw new Error('Nenhum fechamento válido encontrado em Operação Diária.');
}
...
const parameters = Object.fromEntries(
  readSheet('Parâmetros')
    .filter((row) => row.Parâmetro)
    .map((row) => [
      row.Parâmetro,
      {
...
const fuelings = readSheet('Abastecimentos')
  .map((row) => {
    const date = isoDate(row.Data);
...
const expenses = readSheet('Despesas')
  .map((row) => {
    const date = isoDate(row.Data);
    ...
    return {
      ...
      value: round2(row.Valor)
    };
  })
  .filter(Boolean);

const groupSummary = (key) => Object.values(
...
const weekdaySummary = groupSummary('day').map((item) => ({
  day: item.label,
  grossRevenue: round2(item.grossRevenue),
  profit: round2(item.profit),
  hours: round2(item.hours),
  profitPerHour: item.hours ? item.profit / item.hours : 0
}));

const platformSummary = groupSummary('platformMix').map((item) => ({
  platform: item.label,
  revenue: round2(item.grossRevenue),
  trips: item.trips,
  ticketAverage: item.trips ? item.grossRevenue / item.trips : 0
}));
...
const output = {
  meta: {
    ...
    rules: [
      'A planilha MotoristaOps é a fonte operacional.',
      'Todo fechamento diário deve atualizar planilha, JSON e site.',
      'A reserva de manutenção pendente não integra o lucro operacional.'
    ]
  },
  parameters,
  monthly,
